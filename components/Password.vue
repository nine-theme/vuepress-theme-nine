<template lang="pug">
  div(class="password-shadow" :class="{'is-home': !isPage}")
    Background
    h3(class="title") {{isPage ? $frontmatter.title : $site.title}}
    p(class="description" v-if="!isPage") {{$site.description}}
    label(class="inputBox" id="box")
      input(
        ...
        @blur="inputBlur")
      span {{warningText}}
      button(ref="passwordBtn" @click="inter") OK

    div(class="footer")
      span
        i(class="iconfont nine-theme")
        a(target="blank" href="https://www.npmjs.com/package/vuepress-theme-nine") vuePress-theme-nine
      span
        i(class="iconfont nine-other")
        a {{ $themeConfig.author || $site.title }}
      span
...
export default {
  ...
  methods: {
    ...
    inputBlur () {
      this.warningText = 'Duang! Duang! Duang!'
    }
  }
}
</script>
