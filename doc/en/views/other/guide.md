...
3. 你可以打开 [九剑](http://hub.alili.fun) 来查看效果。
:::
...
`vuepress-theme-nine@0.x` 是基于 `vuepress@0.x` 的博客主题。

`vuepress@0.x` 功能比较简单，只适合书写简单的文档，但好在支持主题自定义，而个人又希望能够用它来书写博客，原因就是它足够的简洁，毫无疑问，这也符合很多程序员的观念，也就是在这种情况下，`vuepress-theme-nine@0.x` 的第一个版本经过一个通宵而产生。
...
随着不断有用户过来询问：为什么 `vuepress-theme-nine@0.x` 不能在 `vuepress@1.x` 上使用？本来只是打算对 `vuepress-theme-nine@0.x` 进行简单的bug修复的我，终究还是忍不住，开始了 `vuepress-theme-nine@1.x` 的开发。又是在一个寂静的凌晨两点半（晚上就是出活快），我默默地开始了。
...
`vuepress-theme-nine@0.x` 的开发中，更加深刻地明白了模块化和组件化编程的重要性，如果当初没有把一些功能进行封装，而是直接简单的复制，这次升级也不会这么顺利。模块拆分的越细，使用就会越灵活。
...
## License
[MIT](https://github.com/nine-theme/vuepress-theme-nine/blob/master/LICENSE)
